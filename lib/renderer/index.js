...
import {PrimitiveType} from "../state/primitive_type";
...
export default class Renderer {
     ...
     *       is updated to accept the index type as a parameter or an alternative method is supplied. Just haven't decided yet.
     * @param {PrimitiveType} primitiveType - The type of primitive to be rendered.
     * @param {number} primitiveCount - The number of primitives to be rendered.
    ...
    drawIndexedPrimitive(primitiveType, primitiveCount, startIndex) {
        ...
        switch (primitiveType) {
            case PrimitiveType.PointList:
                this._drawInformation.pointCount += primitiveCount;
                this._gl.drawElements(this._gl.POINTS, primitiveCount, this._gl.UNSIGNED_SHORT, startIndex * 2);
                break;

            case PrimitiveType.LineList:
                this._drawInformation.lineCount += primitiveCount;
                this._gl.drawElements(this._gl.LINES, primitiveCount * 2, this._gl.UNSIGNED_SHORT, startIndex * 2);
                break;

            case PrimitiveType.LineStrip:
                this._drawInformation.lineCount += primitiveCount;
                this._gl.drawElements(this._gl.LINE_STRIP, primitiveCount + 1, this._gl.UNSIGNED_SHORT, startIndex * 2);
                break;

            case PrimitiveType.TriangleList:
                this._drawInformation.triangleCount += primitiveCount;
                this._gl.drawElements(this._gl.TRIANGLES, primitiveCount * 3, this._gl.UNSIGNED_SHORT, startIndex * 2);
                break;

            case PrimitiveType.TriangleStrip:
                this._drawInformation.lineCount += primitiveCount;
                this._gl.drawElements(this._gl.TRIANGLE_STRIP, primitiveCount + 2, this._gl.UNSIGNED_SHORT, startIndex * 2);
                break;
        }
    }
}