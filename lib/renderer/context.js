...
export const WEBGL2_CONTEXT_NAME = 'webgl';
...
export function detectWebGL(canvas, options) {
    ...
    if (options.webgl2) {
        const context = canvas.getContext(WEBGL2_CONTEXT_NAME);
    ...
    if (options.webgl) {
        const context = canvas.getContext(WEBGL_CONTEXT_NAME) || canvas.getContext(EXPERIMENTAL_WEBGL_CONTEXT_NAME);
