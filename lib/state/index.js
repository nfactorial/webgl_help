...
export default class WebGLState {
    ...
    initializeGeometryBuffer(geometryBuffer) {
        if (!geometryBuffer) {
            throw new Error('GeometryBuffer was null');
        }

        // TODO: Need to handle geometry buffers that have no vertex array object

        this.bindVertexArray(null);
        this.bindArrayBuffer(null);

        for (let loop = 0; loop < this.attributeCount; loop++) {
            this._gl.disableVertexAttribArray(loop);
        }
        this.attributeCount = 0;
